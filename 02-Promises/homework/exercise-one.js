...
function problemE () {
  ...
  /*
  promisifiedReadFile('poem-one/stanza-03.txt')
  .then(res => blue(res), err => magenta(new Error(err)));

  promisifiedReadFile('poem-one/wrong-file-name.txt')
    .then(res => blue(res))
    .catch(err2 => magenta(new Error(err2)))
    */

  PromiseRejectionEvent('poem-one/stanza-03.txt') //creo que los parentesis estan mal
    .then( res => { blue(res); 
...
function problemF () {
  ...
  promisifiedReadFile('poem-one/stanza-03.txt')
    .then(res => { blue(res); 
      return promisifiedReadFile('poem-one/wrong-file-name.txt')})
    .then(res2 => blue(res2) /*console.log('done) */)
    .catch(err2 => magenta(new Error(err2)))
